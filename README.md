...
| `@willink-labs/react` | shadcn/ui を semantic token に書き換えた React コンポーネント (**25 components**・Storybook catalog 付き) |
...
### Components shipped (npmjs.org public・25 components)
...
- **v1.9** 🚧 extensibility — コンポーネント増設 + Tailwind-like 拡張 surface + テンプレート/block 配布（3 層: npm primitives 維持 / 公開 `@theme` override 契約 + dark `@custom-variant` + `@layer components` / 自社ホスト shadcn registry `apps/registry` の copy-to-own blocks + GitHub template repos。component catalog-gap を Popover→ScrollArea→Command→Combobox / Table+Pagination / Calendar+DatePicker / nav 系 で closure）: [`docs/roadmap/v1.9.md`](./docs/roadmap/v1.9.md) / [ADR-0020](./docs/adr/0020-distribution-channels.md) · [ADR-0021](./docs/adr/0021-extension-contract.md)
