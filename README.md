...
| `@willink-labs/react` | shadcn/ui を semantic token に書き換えた React コンポーネント (0.5.0・**21 components**) |
| `willink_theme` (pub.dev) | Material 3 ThemeData + brand 軸 (`willink` / `clublink` / `fitai`) + 4 Flutter components (EmptyState / ErrorState / LoadingState / SectionCard・0.3.0) |
...
### Components shipped (0.5.0・npmjs.org public・21 components)

#### Phase 0-3 core set (7 components・0.1.0-0.4.x)
...
#### Phase 7+ expansion (14 components・0.5.0)
...
- **Phase 1** ✅ shadcn 拡張 7 components + npmjs.org publish (v0.2.2 / 2026-05-08・auth 不要)
- **Phase 3** ✅ 最初の consumer 実証 (clublink-platform PR #18 merge / 5/8)
- **Phase 4.1** ✅ i-willink.com 移行 (5/8) + DS 0.4.0 auto-`@source` 仕組み確立
- **Phase 5.1-5.4** ✅ `willink_theme` Flutter package (`WillinkTheme.{willink,clublink,fitai}` ColorScheme・5/8-5/9)・fit-ai mobile + clubhouse 統合 pilot (5/11)
- **Phase 6** ✅ Flutter 4 components (EmptyState / ErrorState / LoadingState / SectionCard・willink_theme 0.3.0・5/11)
- **Phase 7+** ✅ React 14 new components + 200% expansion (Dialog / AlertDialog / Avatar / Tabs / Tooltip / Toast / Dropdown / Select / Switch / Checkbox / RadioGroup / Slider / Progress / Separator・0.5.0・5/11)
- **Phase 4.2.1** (internal-site-1) WP SCSS 段階適用 (`@willink-labs/css-tokens` export package 設計中)
- **Phase 8** OSS public 化判断 (Q3 想定・Code scanning が無料利用可能になるため CodeQL 再有効化)
- **v1.0.0** API 安定化 + 全 consumer 段階展開完了後

> (internal project) ((internal partner) 受託) は Phase 5+ 対象外 (5/8 21:23 JST CEO 確定・(internal ADR))。
> WP 受託サイト ((internal-site-3) / (internal-site-4) / (internal-site-2) / (internal-site-5)) も対象外 ((internal ADR) (2-b) 永久除外)。

詳細 status: [`internal-repo/departments/pm/projects/2026-05-13-design-system-status-roadmap.md`](https://github.com/i-Willink-LLC/internal-repo/blob/main/departments/pm/projects/2026-05-13-design-system-status-roadmap.md)
ADR: [`docs/adr/`](./docs/adr/)

## Contributing

See [CONTRIBUTING.md](./CONTRIBUTING.md) for setup, component guidelines, brand-axis addition flow, and release process. Security vulnerabilities: see [SECURITY.md](./SECURITY.md).
