...
- **Menubar** (Radix `@radix-ui/react-menubar`) — horizontal application menu bar, a near-clone of `ContextMenu` (`Menubar` / `MenubarMenu` / `MenubarTrigger` / `MenubarContent` / `MenubarItem` / `MenubarCheckboxItem` / `MenubarRadioGroup` / `MenubarRadioItem` / `MenubarLabel` / `MenubarSeparator` / `MenubarShortcut` / `MenubarGroup` / `MenubarSub` / `MenubarSubTrigger` / `MenubarSubContent` / `MenubarPortal`). The `Menubar` root is a styled `flex h-10 items-center` bar (`border-border` + `bg-bg`) whose `MenubarTrigger` pills go `data-[state=open]:bg-surface-subtle`; each `MenubarMenu` opens a `role="menu"` panel. `MenubarContent` / `MenubarSubContent` share `ContextMenu`/`DropdownMenu`'s panel (`border-border` + `bg-bg` + `shadow-md`) with `data-[state=open]:animate-fade-in` / `data-[state=closed]:animate-fade-out` + `motion-reduce:animate-none` per [ADR-0008](../../docs/adr/0008-motion-contract.md). Distinct from `NavigationMenu` (site-level `<nav>` disclosure) — this is an app-style action menu bar (File / Edit / View). (#116)
...
- **NavigationMenu** (Radix `@radix-ui/react-navigation-menu`) — site-level navigation menubar (`NavigationMenu` / `NavigationMenuList` / `NavigationMenuItem` / `NavigationMenuTrigger` / `NavigationMenuContent` / `NavigationMenuLink` / `NavigationMenuViewport` / `NavigationMenuIndicator`). A `<nav>` landmark of triggers, each revealing a floating disclosure panel rendered into the shared `NavigationMenuViewport` — distinct from `DropdownMenu` (`role="menu"` action menu). Exports a `navigationMenuTriggerStyle` cva (ghost-like pill: `h-10 px-4 rounded-full`, `hover:`/`focus:`/`data-[state=open]:bg-surface-subtle`, focus-visible ring) used by `NavigationMenuTrigger` (with a `ChevronDown` that `data-[state=open]:rotate-180`) and re-exported so consumers can style a top-level `NavigationMenuLink` to match. `NavigationMenuContent` + `NavigationMenuViewport` are surface panels (`border-border` + `bg-bg` + `shadow-md` + `rounded-md`) carrying `data-[state=open]:animate-fade-in` / `data-[state=closed]:animate-fade-out` + `motion-reduce:animate-none` per [ADR-0008](../../docs/adr/0008-motion-contract.md). (#114)

### Changed

- **Accordion** now re-exports its `AccordionItemProps` / `AccordionTriggerProps` types from the package barrel, matching `Breadcrumb` / `Pagination` / `Sheet` — consumers can `import type { AccordionItemProps } from "@willink-labs/react"`. Additive, non-breaking. (#156)
...
- **`check-story-completeness` gate** (#127) — a static CI test asserting every `src/components/<name>/` dir has an `apps/storybook/src/stories/<name>.stories.tsx` (A ⊆ B). Storybook is the public catalog, so a component shipping without a story is a documentation blind spot. Directional only — non-component overview/intro doc stories are allowed (orphan stories aren't flagged). Same `pnpm -r test` step, no CI change.
- **`check-axe-coverage` gate** (#131) — a static CI test asserting every component's `<name>.test.tsx` contains a real jest-axe assertion (both an `axe(…)` call and `toHaveNoViolations`, after comment-stripping so a commented-out assertion fails). a11y is a core DS promise; `check-a11y-matrix` (#117) guarantees a matrix *row* but not that an axe test actually runs — this closes that. Same `pnpm -r test` step, no CI change.
- **`check-test-presence` gate** (#132) — a static CI test asserting every `src/components/<name>/` dir ships a colocated `<name>.test.tsx`. Pairs with `check-axe-coverage` (#131): this guards the test file's *existence*, that one its *content*. Same `pnpm -r test` step, no CI change.
...
- **Breadcrumb** — zero-dep compound navigation (`Breadcrumb` / `BreadcrumbList` / `BreadcrumbItem` / `BreadcrumbLink` / `BreadcrumbPage` / `BreadcrumbSeparator`). `nav[aria-label="breadcrumb"]` over an `<ol>`; `BreadcrumbLink` supports `asChild` (Radix `Slot`) for router links; `BreadcrumbPage` marks the current page (`role="link"` + `aria-current="page"` + `aria-disabled`); separators are `aria-hidden` (default lucide `ChevronRight`). Semantic tokens only. (#95)
- **ContextMenu** (Radix `@radix-ui/react-context-menu`) — right-click context menu, a near-clone of `DropdownMenu` (`ContextMenu` / `ContextMenuTrigger` / `ContextMenuContent` / `ContextMenuItem` / `ContextMenuLabel` / `ContextMenuSeparator` / `ContextMenuGroup` / `ContextMenuSub` / `ContextMenuRadioGroup` / `ContextMenuPortal` / `ContextMenuShortcut`). `ContextMenuContent` shares DropdownMenu's panel + `motion-reduce:animate-none` ([ADR-0008](../../docs/adr/0008-motion-contract.md)). (#95)
